...
from boxing_project.tracking.inference_utils import init_yolo_from_config, visualize_sequence
...
class InferRunner:
    ...
    def run(self) -> None:
        ...
        if "yolo" not in cfg:
            raise KeyError("infer_tracks.yaml missing key: yolo")
        yolo_cfg = dict(cfg["yolo"])
        yolo_model_path = _resolve(pr, yolo_cfg.get("model_path"))
        if yolo_model_path is None or not yolo_model_path.exists():
            raise FileNotFoundError(f"YOLO model not found: {yolo_model_path}")
        yolo_cfg["model_path"] = str(yolo_model_path)
        detector = init_yolo_from_config(yolo_cfg)
